...
def test_run_feasibility_audit_emits_sleeve_gate_false_without_series(tmp_path) -> None:
    import json
    from datetime import date
    from pathlib import Path
    ...
    assert "sticky.mom60_raw" in result["strategies"]
    metrics = json.loads((out / "2026_championship_feasibility_metrics.json").read_text(encoding="utf-8"))
    assert metrics["championship_sleeve_is_production_gate"] is False
    assert metrics["activation_state_is_production_gate"] is False
    assert metrics["audit_regime_is_activation_gate"] is False
    assert not (out / "championship_sleeve_table.csv").exists()


def test_run_feasibility_audit_writes_championship_sleeve_table(tmp_path) -> None:
    import json
    from datetime import date
    from pathlib import Path
    ...
    p27 = pl.DataFrame({"window_start": [date(2026, 1, 5), date(2026, 1, 6)], "terminal_return": [0.51, 0.10], "giveback": [0.02, 0.01]})
    b1 = pl.DataFrame({"window_start": [date(2026, 1, 5), date(2026, 1, 6)], "terminal_return": [0.02, 0.03], "giveback": [0.01, 0.01]})
    mom60 = {sessions[i]: v for i, v in enumerate([0.01, 0.02, -0.22, 0.12, 0.04])}
    mom20 = {sessions[i]: v for i, v in enumerate([0.01, 0.02, 0.23, 0.04, 0.01])}
    rv = {sessions[i]: v for i, v in enumerate([0.01, 0.01, 0.056, 0.01, 0.01])}
    out = Path(tmp_path) / "research"
    result = run_feasibility_audit(
        calendar_sessions=sessions,
        panel=panel,
        ...
        manifest=None,
        enforce_horizon_36=False,
        kospi_mom60_by_date=mom60,
        kospi_mom20_by_date=mom20,
        kospi_rv20_daily_by_date=rv,
    )
    assert result["strategies"]["sticky.mom60_raw"]["n"] == 2
    metrics = json.loads((out / "2026_championship_feasibility_metrics.json").read_text(encoding="utf-8"))
    assert metrics["championship_sleeve_is_production_gate"] is False
    table = pl.read_csv(out / "championship_sleeve_table.csv")
