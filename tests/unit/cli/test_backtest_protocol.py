...
def test_SCENARIO_11_03_cli_protocol_defaults() -> None:  # noqa: N802
    ...
    args_grid = parser.parse_args(
        ["backtest", "--model", "baseline.mom20_top1", "--start", "2024-01-02", "--end", "2024-01-08", "--protocol", "grid"]
    )
    assert args_grid.protocol == "grid"

    args_stress = parser.parse_args(
        ["backtest", "--model", "baseline.mom20_top1", "--start", "2024-01-02", "--end", "2024-01-08", "--stress-grid"]
    )
