    ...
    *,
    ...
    if p27_regime_inputs_by_date is not None and sticky_aligned is not None and len(executable_pop.windows) > 0:
        ...
        for window in executable_pop.windows:
            p27_snap = classify_p27_regime(decision_date=window.decision_date, inputs=p27_regime_inputs_by_date.get(window.decision_date))
        ...
        p27_table = p27_regime_conditional_table(states=p27_states, confidences=p27_confidences, terminal_returns=sticky_aligned, overlap_horizon=horizon)
        p27_table.write_csv(output_dir / "p27_regime_confidence_table.csv")
        last = p27_snaps[-1]
        metrics["p27_regime_as_of"] = {"date": last.as_of.isoformat(), "state": last.state.value, "confidence": float(last.confidence), "components": dict(last.components)}
